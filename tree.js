//SOURCE: https://github.com/lunu-bounir/tree.js 
...
class SimpleTree extends Emitter {
  ...
  file(node, parent = this.parent, before) {
    ...
    const a = this.append(Object.assign(document.createElement('a'), {
      textContent: this.shortenName(node.name),
      href: '#',
      name: node.name
    }), parent, before);
    a.title = this.shortenName(node.name);
  ...
  folder(node, parent = this.parent, before) {
    ...
    details.title = this.shortenName(node.name);
    const summary = Object.assign(document.createElement('summary'), {
      innerHTML: '<div class="text-truncate" style="margin-left: 0px; width: 280px;">' + this.shortenName(node.name) + '<\div>',
      name: node.name
    });
